...
function topFunction() {
  ...
  document.documentElement.scrollTop = 0;
	...
	<div id = "position">
		<ul>
			<li><button id="btn1" onclick = "ProductListOuter"> OUTER </button></li>
	...
	<div id="paging">
		...
		<c:if test="${paging.page>1}">
			<a href="ProductListOuter?page=${paging.page-1}">[이전]</a>&nbsp;
		</c:if>
		<c:forEach begin="${paging.startPage}" end="${paging.endPage}" var="i"
			...
			<c:choose>
				...
				<c:otherwise>
					<a href="ProductListOuter?page=${i}">${i}</a>
				</c:otherwise>
			</c:choose>
		</c:forEach>
		...
		<c:if test="${paging.page<paging.maxPage}">
			&nbsp;<a href="ProductListOuter?page=${paging.page+1}">[다음]</a>
		</c:if>
	</div>
	<br><br>
	<div id = "footerdiv">
